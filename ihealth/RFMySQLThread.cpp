...
int RFMySQLThread::Connect(EventArg *pArg)
{
	...
	cgf.read("../../ihealth/resource/params/mysql.cfg");
		...
		if (stmt1.Prepare(RFMainWindow::DBThread->m_db, szSQL) > 0) {
			...
			if (stmt1.Step() > 0) {
				...
				*/
				...
				char sz[1024];
				sprintf(sz, "%.3f%s", treattime, TGUTF16ToUTF8(_T("小时")).c_str());
...
int RFMySQLThread::LoadPassiveTrainDocumentInfo() {
	string passive_document_path = "../passivetrain/passivetrain.txt";
	ReadPassiveTrainDocument(passive_document_path);



	//CTask::Assign(CTask::NotWait, Panic(), pResult, EventHandle(&RFPassiveTrain::OnLoadPassiveTrainDocumentInfoOK), pCurrentThread, RFMainWindow::UIThread);

	return 1;
}

void RFMySQLThread::ReadPassiveTrainDocument(string file) {
	ifstream infile;
