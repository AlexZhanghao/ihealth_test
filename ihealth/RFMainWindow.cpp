...
void RFMainWindow::BindManagerPatientPageEvent()
{
	...
	CButtonUI* load_doucment = static_cast<CButtonUI*>(m_pm.FindControl(_T("load_doucment")));
	load_doucment->OnNotify += MakeDelegate(this, &RFMainWindow::LoadAction);
...
bool RFMainWindow::LoadAction(void *pParam) {
	TNotifyUI *pMsg = static_cast<TNotifyUI*>(pParam);
	if (pMsg->sType != _T("click"))
		return true;

	loadeasy.LoadPassiveTrainDocumentInfo();
	return true;
}
